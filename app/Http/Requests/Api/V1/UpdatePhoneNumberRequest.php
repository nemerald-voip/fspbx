...
class UpdatePhoneNumberRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $destinationUuid = (string) $this->route('destination_uuid');
        ...
        return [
            'destination_number' => [
                ...
                'string',
                'regex:/^\+?\d+$/',
                Rule::unique('v_destinations', 'destination_number')
                    ->where(fn ($q) => $q->where('domain_uuid', $domainUuid))
                    ->ignore($destinationUuid, 'destination_uuid'),
            ],

            'destination_prefix' => ['sometimes', 'nullable', 'string', 'max:5'],

            'destination_description' => ['sometimes', 'nullable', 'string', 'max:150'],

            'destination_record' => ['sometimes', 'boolean'],
            'destination_type_fax' => ['sometimes', 'boolean'],

            'destination_hold_music' => ['sometimes', 'nullable', 'string', 'max:120'],
            'destination_distinctive_ring' => ['sometimes', 'nullable', 'string', 'max:30'],
            'destination_cid_name_prefix' => ['sometimes', 'nullable', 'string', 'max:20'],
            'destination_accountcode' => ['sometimes', 'nullable', 'string', 'max:50'],
            'fax_uuid' => ['sometimes', 'nullable', 'uuid', Rule::exists('v_fax', 'fax_uuid')],
