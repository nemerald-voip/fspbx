...
{
    private const DEFAULT_LOG_DIR = '/var/log/nginx';
    private const DEFAULT_LOG_FILE = 'access.log';
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $logDirectory = self::DEFAULT_LOG_DIR;
        ...
        if ($approvedFiles->isEmpty()) {
            return response()->json([
                ...
                'lines' => [],
                'meta' => [
                    'errors' => [__('No nginx log files were found.')],
        ...
        $validated = $request->validate([
            ...
            'level' => ['nullable', 'string', 'in:all,info,notice,warning,err'],
    ...
    {
        return (bool) preg_match('/^(?:access|error)\.log(?:\.\d+)?$/', $basename);
    }
    ...
    private function searchFiles(
        ...
        foreach ($files as $file) {
            ...
            while (($lineOffset = ftell($handle)) !== false && ($line = fgets($handle)) !== false) {
                ...
                $line = rtrim($line, "\r\n");
                $parsed = $this->parseLine($line, $file['basename']);

                if ($this->isViewerRequestLine($line)) {
                    continue;
                }
    ...
    private function isViewerRequestLine(string $line): bool
    {
        return (bool) preg_match('/"\S+\s+\/api\/nginx-logs(?:[?\s"])/', $line);
    }

    private function parseLine(string $line, string $basename): array
    {
        preg_match('/^(\d{4}\/\d{2}\/\d{2}\s+\d{2}:\d{2}:\d{2})\s+\[(emerg|alert|crit|error|warn|notice|info|debug)\]/i', $line, $errorMatch);

        if ($errorMatch) {
            return [
                'timestamp' => str_replace('/', '-', $errorMatch[1]),
                'level' => $this->normalizeErrorLevel($errorMatch[2]),
            ];
        }

        preg_match('/\[(\d{2}\/[A-Za-z]{3}\/\d{4}:\d{2}:\d{2}:\d{2}\s+[+-]\d{4})\]/', $line, $accessTimeMatch);
        preg_match('/"\s+(\d{3})\s+/', $line, $statusMatch);

        return [
            'timestamp' => $accessTimeMatch[1] ?? null,
            'level' => $this->levelFromStatus($statusMatch[1] ?? null, $basename),
        ];
    }

    private function normalizeErrorLevel(string $level): string
    {
        return match (strtolower($level)) {
            'emerg', 'alert', 'crit', 'error' => 'err',
            'warn' => 'warning',
            'notice' => 'notice',
            default => 'info',
        };
    ...
    private function levelFromStatus(?string $status, string $basename): string
    {
        if ($status === null) {
            return str_starts_with($basename, 'error.log') ? 'err' : 'info';
        }
