...
class VoxutelWebhookParser implements MessagingWebhookParser
{
    ...
    public function parse(WebhookCall $webhookCall): iterable
    {
        ...
        messaging_webhook_debug('Voxutel parser received payload', [
            'type' => $type,
        ...
        if ($this->isInboundMessagePayload($payload)) {
            ...
            messaging_webhook_debug('Voxutel parser yielding inbound event', [
        ...
        if ($this->isDeliveryReceiptPayload($payload)) {
            ...
            messaging_webhook_debug('Voxutel parser yielding status event', [
        ...
        messaging_webhook_debug('Voxutel parser found no matching payload shape');
    }

    public function downloadMedia(string $url): DownloadedMediaData
    {
        messaging_webhook_debug('Voxutel downloadMedia started', ['url' => $url]);
        ...
        messaging_webhook_debug('Voxutel downloadMedia completed', [
            'url' => $url,
    ...
    protected function isInboundMessagePayload(array $payload): bool
    {
        ...
        if (data_get($payload, 'deliveryReceipt') === false
            && filled($this->extractFrom($payload))
            && ! empty($this->extractTo($payload))) {
            return true;
        }

        // Direct deliveries from Voxutel Wholesale use messageId/messageType and
        // omit Sinch's deliveryReceipt field.
        return filled(data_get($payload, 'messageId'))
            && filled($this->extractFrom($payload))
            && ! empty($this->extractTo($payload));
    }
    ...
    protected function extractMediaUrls(array $payload): array
    {
        ...
        return collect(Arr::wrap($media))
            ...
            ->filter(fn ($item) => filled($item))
            ->reject(fn ($url) => $this->shouldSkipVoxutelMediaUrl($url))
            ->values()
            ->all();
    }

    protected function shouldSkipVoxutelMediaUrl(string $mediaUrl): bool
    {
