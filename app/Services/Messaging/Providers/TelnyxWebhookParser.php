...
class TelnyxWebhookParser implements MessagingWebhookParser
{
    /**
    ...
    public function parse(WebhookCall $webhookCall): iterable
    {
        ...
        messaging_webhook_debug('TelnyxWebhookParser parse() started', [
        ...
        if (! is_array($payload)) {
            messaging_webhook_debug('TelnyxWebhookParser invalid payload shape', [
                'event_type' => $eventType,
            ]);
        ...
        if ($eventType === 'message.received') {
            ...
            $event = InboundMessageEventData::from([
                'provider' => 'telnyx',
                'providerReferenceId' => (string) ($payload['id'] ?? ''),
            ...
            messaging_webhook_debug('TelnyxWebhookParser inbound event parsed', [
        ...
        if ($eventType === 'message.finalized') {
            $providerStatus = strtolower((string) data_get($payload, 'to.0.status', ''));

            $event = DeliveryStatusEventData::from([
                'provider' => 'telnyx',
                'referenceId' => (string) ($payload['id'] ?? ''),
                'status' => $this->mapDeliveryStatus($providerStatus),
                'description' => $this->extractDeliveryDescription($payload, $providerStatus),
                'providerEvent' => $eventType,
            ]);

            messaging_webhook_debug('TelnyxWebhookParser delivery event parsed', [
        ...
        messaging_webhook_debug('TelnyxWebhookParser ignored event', [
            'event_type' => $eventType,
        ]);
    }
    ...
    {
        messaging_webhook_debug('TelnyxWebhookParser downloadMedia() started', [
            'url' => $url,
        ]);
        ...
        messaging_webhook_debug('TelnyxWebhookParser downloadMedia() completed', [
            'url' => $url,
            'mime_type' => $mimeType,
            'size' => strlen($binary),
    ...
    protected function mapDeliveryStatus(string $providerStatus): string
    {
        return match ($providerStatus) {
            'delivered' => 'delivered',
            'sending_failed', 'delivery_failed', 'delivery_unconfirmed', 'failed', 'gw_timeout', 'dlr_timeout' => 'failed',
            default => 'failed',
        };
    }
    ...
    {
        $firstError = data_get($payload, 'errors.0');
