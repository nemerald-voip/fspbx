...
class VoxutelOutboundProvider implements OutboundProviderInterface
{
    public function send(Messages $message): OutboundSendResultData
    {
        ...
        messaging_webhook_debug('VoxutelOutboundProvider request payload', [
            'message_uuid' => $message->message_uuid,
            'payload' => $payload,
        ]);

        $response = Http::withToken(config('voxutel.api_key'))
            ->asJson()
            ->timeout(30)
            ->post(config('voxutel.message_broker_url'), $payload);
        $result = $response->json();

        messaging_webhook_debug('VoxutelOutboundProvider response received', [
            'message_uuid' => $message->message_uuid,
    ...
    private function buildMediaUrls(Messages $message): array
    {
        return collect($message->media ?? [])
            ->map(function ($item) {
                ...
                return is_array($item) ? ($item['url'] ?? null) : null;
            })
            ->filter()
            ->unique()
            ->values()
