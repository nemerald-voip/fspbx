    ...
    public function up(): void
    {
        if (!Schema::hasTable('provisioning_templates')) {
            Schema::create('provisioning_templates', function (Blueprint $t) {
                ...
                $t->string('vendor')->nullable();               // poly|yealink|grandstream|dinstar
                $t->string('name')->nullable();                 // display name (e.g., PolyVVX)

                $t->string('type')->nullable()->default('default');

                // ----- Default releases -----
                $t->string('version')->nullable();  // SemVer string, e.g., "1.0.8" (defaults only)

                // ----- Custom tracking -----
                $t->integer('revision')->nullable()->default(0);    // bump on every edit of a custom
                ...
                $t->string('checksum', 64)->nullable()->index();   // sha256(content)
