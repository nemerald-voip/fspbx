                                    ...
                                    <div class="lg:grid lg:grid-cols-12 lg:gap-x-5">
                                        <div class="px-2 py-6 sm:px-6 lg:col-span-3 lg:px-0 lg:py-0">
                                            <FormTabs view="vertical">
                                                <FormTab name="settings" :label="$t('Settings')" :elements="settingsElements" />
                                        ...
                                        <div class="space-y-6 bg-gray-50 px-4 py-6 text-gray-600 shadow sm:rounded-md sm:px-6 sm:p-6 lg:col-span-9">
                                            <FormElements>
                                                ...
                                                <StaticElement name="settings_header" tag="h4" :content="$t('Stream Settings')"
                                                    :description="$t('Configure the stream category, source path, and playback behavior.')" />

                                                <TextElement name="music_on_hold_name" :label="$t('Name')" placeholder="default" :floating="false" :columns="{ sm: { container: 6 } }" />
                                                <SelectElement v-if="canManageDomain" name="domain_uuid" :items="domainOptions" :label="$t('Domain')" :native="false" :floating="false" :columns="{ sm: { container: 6 } }" />

                                                <SelectElement name="music_on_hold_channels" :items="channelOptions" :label="$t('Channels')" :native="false" :floating="false" :columns="{ sm: { container: 6 } }" />
                                                <ToggleElement name="music_on_hold_shuffle" :text="$t('Shuffle')" true-value="true" false-value="false" :labels="{ on: 'On', off: 'Off' }" :columns="{ sm: { container: 6 } }" label="&nbsp;" />
                                                ...
                                                <StaticElement v-if="canViewPath" name="music_on_hold_path" tag="div" :columns="{ container: 12 }">
                                                    <div class="rounded-md bg-gray-100 px-3 py-2">
                                                        <div class="text-xs font-medium uppercase tracking-wide text-gray-500">{{ $t('Generated path') }}</div>
                                                ...
                                                <StaticElement name="advanced_header" tag="h4" :content="$t('Advanced Settings')"
                                                    :description="$t('Optional local stream settings passed to FreeSWITCH.')" />

                                                <TextElement name="music_on_hold_interval" input-type="number" :label="$t('Interval')" :floating="false" :columns="{ sm: { container: 4 } }" />
                                                <TextElement name="music_on_hold_timer_name" :label="$t('Timer Name')" :floating="false" :columns="{ sm: { container: 4 } }" />

                                                <SelectElement name="music_on_hold_chime_list" :items="chimeOptions" :groups="true" :search="true" :native="false"
                                                    :label="$t('Chime')" input-type="search" allow-absent autocomplete="off" :placeholder="$t('No chime')"
                                                    :floating="false" :strict="false" :columns="{ sm: { container: 4 } }" />

                                                <TextElement name="music_on_hold_chime_freq" input-type="number" :label="$t('Chime Frequency')" :floating="false" :columns="{ sm: { container: 6 } }" />
                                                <TextElement name="music_on_hold_chime_max" input-type="number" :label="$t('Chime Maximum')" :floating="false" :columns="{ sm: { container: 6 } }" />
...
const channelOptions = computed(() => [
    { label: trans('Mono'), value: "1" },
    { label: trans('Stereo'), value: "2" },
]);
