<template>
    <div class="bg-white px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 gap-6">
            <div>
                <LabelInputRequired target="emergency_number" :label="$t('Emergency Number')" />
                <div class="mt-2">
                    <InputField v-model="form.emergency_number" type="text" name="emergency_number"
                        :placeholder="$t('Enter number (e.g. 911, 112)')" />
                </div>
            ...
            <div>
                <LabelInputOptional target="members" :label="$t('Extensions to Notify')" />
                <div class="mt-2 relative">
                    <Multiselect v-model="form.members" :options="options.extensions" :multiple="true"
                        :close-on-select="false" :clear-on-select="false" :preserve-search="true"
                        :placeholder="$t('Choose Extensions')" label="name" track-by="value" :searchable="true" />

                    <div class="mt-1 text-sm text-gray-500">
                        {{ $t('Selected extensions will be called and notified when an emergency number is dialed.') }}
                    </div>
                </div>
            ...
            <div>
                <LabelInputOptional target="emails" :label="$t('Emails to Notify')" />
                <textarea v-model="form.emails" rows="3"
                    :placeholder="$t('Enter emails separated by commas (e.g. admin@example.com, support@example.com)')"
