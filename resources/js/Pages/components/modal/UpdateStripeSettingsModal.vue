                        ...
                        <DialogPanel
                            ...
                            <div class="flex items-start justify-between border-b border-gray-100 px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 class="text-base font-semibold text-gray-900">{{ $t('Stripe settings') }}</h3>
                                        <p class="text-xs text-gray-500">{{ $t('Connect your Stripe account for billing & quotes.') }}</p>
                                    </div>
                                </div>
                            ...
                            <Vueform ref="form$" :endpoint="submitForm" @success="handleSuccess" @error="handleError"
                                @response="handleResponse" :display-errors="false" class="px-6 py-5" :default="defaults" @mounted="(form) => form.disableValidation()" @submit="clearServerFormErrors">

                                <HiddenElement name="uuid" :meta="true" />

                                <!-- enable + mode -->
                                <ToggleElement name="status" :text="$t('Gateway enabled')" true-value="true" false-value="false" />
                                <RadiogroupElement name="sandbox" :label="$t('Mode')" view="tabs" :items="{ true: $t('Test'), false: $t('Live') }"
                                    :description="$t('Test uses sandbox keys and never charges a real card.')" />
                                ...
                                <StaticElement name="reveal_secrets">
                                    <label class="flex cursor-pointer items-center gap-2 text-xs font-medium text-gray-500">
                                        ...
                                        {{ $t('Show secret values') }}
                                    </label>
                                </StaticElement>

                                <!-- TEST keys -->
                                <StaticElement name="test_keys_heading" tag="p"
                                    :content="$t('Test API keys')" :conditions="[['sandbox', 'true']]"
                                    class="text-xs font-semibold uppercase tracking-wide text-gray-400" />
                                <TextElement name="sandbox_secret_key" :label="$t('Secret key')"
                                ...
                                <TextElement name="sandbox_publishable_key" :label="$t('Publishable key (optional)')"
                                    :floating="false" placeholder="pk_test_…" :conditions="[['sandbox', 'true']]" />

                                <!-- LIVE keys -->
                                <StaticElement name="live_keys_heading" tag="p"
                                    :content="$t('Live API keys')" :conditions="[['sandbox', 'false']]"
                                    class="text-xs font-semibold uppercase tracking-wide text-gray-400" />
                                <TextElement name="live_mode_secret_key" :label="$t('Secret key')"
                                    :input-type="showSecrets ? 'text' : 'password'" autocomplete="off" :floating="false"
                                    placeholder="sk_live_…" :description="hint('live_mode_secret_key', 'sk_live_…')"
                                    :conditions="[['sandbox', 'false']]" />
                                <TextElement name="live_mode_publishable_key" :label="$t('Publishable key (optional)')"
                                ...
                                <StaticElement name="test_connection">
                                    <div class="space-y-2 rounded-md bg-gray-50 p-3 ring-1 ring-gray-200">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span v-if="!testResult" class="text-xs text-gray-400">{{ $t('Verifies the key against Stripe before you save.') }}</span>
                                        </div>
                                ...
                                <!-- webhook -->
                                <StaticElement name="webhook_heading" tag="p" :content="$t('Webhook')"
                                    class="text-xs font-semibold uppercase tracking-wide text-gray-400" />
                                <StaticElement name="webhook_endpoint">
                                    <div class="text-xs text-gray-500">
                                        {{ $t('Add this endpoint in Stripe → Developers → Webhooks:') }}
                                ...
                                <TextElement name="webhook_secret" :label="$t('Secret')"
                                    :input-type="'text'" autocomplete="off" :floating="false"
                                    placeholder="whsec_…" :description="$t('Used to verify incoming Stripe webhooks.')" />

                                <!-- actions -->
                                <ButtonElement @click="emit('close')" name="cancel" :button-label="$t('Cancel')" :secondary="true"
                                    :columns="{ container: 6 }" :full="true" />
                                <ButtonElement name="submit" :button-label="$t('Save settings')" :submits="true" :full="true"
...
const hint = (key, example) => {
    ...
    return saved ? trans('Saved: :value — leave blank to keep it', { value: saved }) : trans('Enter your :example key', { example });
};
...
const runTest = async () => {
    ...
    try {
        ...
        testResult.value = { ok: false, message: e?.response?.data?.message || trans('Test failed.') };
    } finally {
        testing.value = false;
    }
};
