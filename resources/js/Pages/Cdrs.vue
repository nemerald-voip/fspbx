...
<template>
  <Menu
    :menus="menus"
    :domain-select-permission="domainSelectPermission"
  ...
  <div class="m-3">
    <DataTable
      ...
      @search-action="handleSearchButtonClick"
    >
      ...
      <template #table-body>
        <tr v-for="row in data" :key="row.xml_cdr_uuid">
          ...
          <TableField class="whitespace-nowrap px-2 py-1 text-sm text-gray-500">
            <template
              v-if="
                (row.record_name && row.record_path) || row.record_path === 'S3'
              "
              #action-buttons
            >
              <div class="flex items-center space-x-2 whitespace-nowrap">
                <PlayCircleIcon
                  v-if="
                    currentAudioUuid !== row.xml_cdr_uuid || !isAudioPlaying
                  "
                ...
                />
                <PauseCircleIcon
                  v-if="currentAudioUuid === row.xml_cdr_uuid && isAudioPlaying"
                  @click="pauseAudio"
                  class="h-6 w-6 text-blue-500 hover:text-blue-700 active:h-5 active:w-5 cursor-pointer"
                />
    ...
    <div v-if="data.length === 0" class="text-center mt-2">
